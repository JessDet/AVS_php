...
$stateRead = $pdo->prepare("SELECT titre,categorie,descriptif, dateAjout
    FROM souhait
    WHERE idUser=$user[idUser] and souhait.status=1");
$stateRead->execute();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $State = $pdo->prepare("SELECT titre,categorie,descriptif, dateAjout
    FROM souhait
    ON idUser = $user[idUser] WHERE categorie:?");
    $stateSelect->bindValue(':?, $categorie');
    $stateSelect->execute();
    $selectedCategorie = $stateSelect->fetchAll();
}
...
<head>
    ...
    <link rel="stylesheet" href="/public/css/myWishes.css">
...
<main class="container">
        ...
        <div class="encours">
            <H1 class="title">MES ARCHIVES</H1>
        </div>
        ...
        <div id="categories">
            <ul class="category-container">
                ...
                <li class="fz">
                    <a class="cat" href=" /archives.php" style="color:white;">Tous les Souhaits<span class="small">(<?= count($allWishes) ?>)</span></a>
                </li>
                <?php foreach ($cat ??[] as $cKey => $cNum) : ?>
                    <li class="fz"> 
                        <a class="cat" style="color:white;" href="/archives.php?cat=<?= $cKey ?>"><?= $cKey ?><span class="small">(<?= $cNum ?>)</span></a>
                    </li>
                <?php endforeach; ?>
        ...
        <article class=" content">
            ...
            <?php foreach ($artPerCat[$c] as $a) : ?>
                ...
                <a href="/myWishesContenu.php?id=<?= $a['idSouhait'] ?>" class="blocsouhait"><button class="btnContenu">
                <div class="wish_container">
                    ...
                    <section class="low">
                        <div class="title_description">
                            <h2><?= $a['titre'] ?? '' ?></h2>
                            <p class="descriptif"><?= $a['descriptif'] ?? '' ?></p>
                        </div>
                    </section>
                </div>
            </button></a>
